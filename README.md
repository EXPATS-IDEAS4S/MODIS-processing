...
- `scripts/1_process_modis.py`
  ...
  - Processing options (config-driven):
    ...
    - `processing.compression_level` (int): NetCDF compression level from `0` to `9` when writing outputs. Lower values are faster; default `4`.
    - `processing.output_base_path`: output folder base for NetCDF files.
  - Output contents and format:
    - The produced NetCDF contains only the two BT variables (one per configured channel), `cloud_mask`, and geolocation coordinates.
    - In regular-grid mode the coordinates are 1D `latitude` and `longitude` axes; in native-grid mode they are 2D `latitude(y, x)` and `longitude(y, x)` coordinates attached to the swath.
    ...
    - Files are written with NetCDF compression using `processing.compression_level`.
  - Output folder structure is `year/month/day`.

- `scripts/2_upload_s3.py`
...
- `scripts/run_pipeline.py`
  - Orchestrates the full pipeline one day at a time: download -> processing -> upload.
  - Calls the numbered stage scripts directly and streams all output into one log file.
  - Logs per-batch timing, download/processing/upload counts, and failures.
...
- years: `[2024]`
- months: `[4, 5, 6, 7, 8, 9]`
- days: `all` or a list such as `[1, 5, 15, 28]`
...
```bash
  python scripts/0_download_modis.py --config config/pipeline_config.yaml
```

Single day:

```bash
  python scripts/0_download_modis.py --config config/pipeline_config.yaml --date 2024-04-15
```

Dry run:

```bash
  python scripts/0_download_modis.py --config config/pipeline_config.yaml --dry-run
```
...
```bash
  python scripts/1_process_modis.py --config config/pipeline_config.yaml --date 2024-04-15
```

Dry run (process only first available day):

```bash
  python scripts/1_process_modis.py --config config/pipeline_config.yaml --dry-run
```

### 3) Upload only

```bash
  python scripts/2_upload_s3.py --config config/pipeline_config.yaml --credentials s3_credentials.py --verify
```

Single day:

```bash
  python scripts/2_upload_s3.py --config config/pipeline_config.yaml --credentials s3_credentials.py --date 2024-04-15
```
...
This runs daily batches using the `pipeline` toggles in config:
- `run_download`
- `run_processing`
- `run_upload`
- `verify_upload`

The selected period comes from `years`, `months`, and `days`. Use `days: all` to keep the full month span, or provide explicit day numbers.

Download tuning options live under `download`:
- `download.parallel`: enable threaded downloads for matching granules.
- `download.workers`: number of download worker threads.
- `download.min_overlap_fraction`: skip granules whose reported footprint overlaps the ROI by less than this fraction.

The unified pipeline log is written to `processing.output_base_path/pipeline.log` by default. You can override it with `--log-file`.
...
- Regridding uses cached interpolation geometry per source grid, so BT channels and cloud mask can reuse the same triangulation when they share geolocation.
...
- NetCDF files are written with configurable compression. Lower levels are faster; higher levels shrink files more.
